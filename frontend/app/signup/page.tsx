...
export default function SignupPage() {
  ...
  return <main className="auth-shell"><section className="auth-card signup-card">
    ...
    <form className="auth-form signup-form" onSubmit={submit}><div className="signup-grid">
      <label>이름 <span className="required-mark">필수 · 한글</span><input value={form.name} onChange={(e) => update("name", e.target.value)} pattern="[가-힣]+" maxLength={100} autoComplete="name" required /></label>
      <label>사원번호(ID) <span className="required-mark">필수 · 한글/영문/숫자</span><input value={form.employee_number} onChange={(e) => update("employee_number", e.target.value)} pattern="[가-힣A-Za-z0-9]+" maxLength={30} autoComplete="username" required /></label>
      <label>비밀번호 <span className="required-mark">필수 · 영문/숫자 8자 이상</span><input type="password" value={form.password} onChange={(e) => update("password", e.target.value)} pattern="[A-Za-z0-9]{8,128}" minLength={8} maxLength={128} autoComplete="new-password" required /></label>
      <label>비밀번호 확인 <span className="required-mark">필수</span><input type="password" value={form.confirm} onChange={(e) => update("confirm", e.target.value)} pattern="[A-Za-z0-9]{8,128}" minLength={8} maxLength={128} autoComplete="new-password" required /></label>
      ...
      <label>소속 부서 <span className="optional-mark">선택 · 한글/영문</span><input value={form.department} onChange={(e) => update("department", e.target.value)} pattern="[가-힣A-Za-z ]+" maxLength={100} autoComplete="organization" /></label>
      <label>직책 <span className="optional-mark">선택 · 한글/영문</span><input value={form.job_title} onChange={(e) => update("job_title", e.target.value)} pattern="[가-힣A-Za-z ]+" maxLength={100} autoComplete="organization-title" /></label>
