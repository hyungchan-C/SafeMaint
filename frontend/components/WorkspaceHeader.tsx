  ...
  return (
    <header className="workspace-header">
      ...
      <nav className="workspace-header-actions" aria-label="빠른 기능">
        <button type="button" className={isRecording ? "header-action desktop-header-action active" : "header-action desktop-header-action"} onClick={onVoiceInput} disabled={isTranscribing}>
          <InterfaceIcon name={isRecording ? "stop" : "microphone"} />
          <span>{isRecording ? "녹음 중지" : isTranscribing ? "인식 중" : "음성 입력"}</span>
        </button>
        <button type="button" className={isSpeaking ? "header-action desktop-header-action active" : "header-action desktop-header-action"} onClick={onSpeakGuidance}>
          <InterfaceIcon name={isSpeaking ? "stop" : "speaker"} />
          <span>{isSpeaking ? "음성 중지" : "음성 안내"}</span>
        </button>
        <button type="button" className="header-action desktop-header-action" onClick={onHistory}>
        ...
        <details className="settings-control">
          ...
          <aside className="settings-popover">
            ...
            <div className="mobile-settings-actions" aria-label="모바일 빠른 기능">
              <button type="button" onClick={onVoiceInput} disabled={isTranscribing}>{isRecording ? "녹음 중지" : isTranscribing ? "음성 인식 중" : "음성 입력"}</button>
              <button type="button" onClick={onSpeakGuidance}>{isSpeaking ? "음성 안내 중지" : "음성 안내"}</button>
              <button type="button" onClick={onHistory}>대화 기록</button>
              <button type="button" onClick={onAssessments}>저장된 체크리스트</button>
            </div>
