    ...
    <section className="structured-section maintenance-checklist">
      ...
      {items.length > 0 ? (
        <div className="chat-checklist-items">
          {items.map((item) => (
          <label className={checked.has(item.sequence) ? "checked" : ""} key={`${item.sequence}-${item.content}`}>
            <input
              type="checkbox"
              checked={checked.has(item.sequence)}
              onChange={(event) => setChecked((current) => {
                const next = new Set(current);
                if (event.target.checked) next.add(item.sequence);
