...
SigLIP2-base NaFlex가 PDF에서 추출한 제품 이미지와 원본·다중 확대 현장 사진의 외형 후보를 검색하고,
Qwen3-VL-2B-Instruct가 이미지 구조화와 후보 의미 검토를 수행합니다. 모델은 모두 로컬에
저장되며 OpenAI API 키를 사용하지 않습니다.

사진 첨부는 2단계로 동작합니다. 먼저 `analysis_mode=fast`가 원본과 겹치는 확대 영역을
SigLIP2 배치 임베딩으로 일괄
...
EfficientNet 계열 등 이전 형식으로 만든 비전 인덱스는 SigLIP2 행렬 인덱스와 벡터 차원이 다르므로 그대로 검색하지 않습니다. 기존 문서에서 사진 검색을 사용하려면 문서 목록의 **비전 재인덱싱** 버튼을 한 번 실행해야 합니다. 이 작업은 기존 `POST /api/v1/vision/catalog/index`를 사용하며 `document.upload` 권한, 문서 소유권 및 사업장 접근 범위 검사를 우회하지 않습니다. 새 인덱스에는 `vision_catalog_index_version`이 기록되고, 버전 정보가 없는 기존 인덱스로 사진 분석을 요청하면 재인덱싱 안내와 함께 `409`를 반환합니다.
