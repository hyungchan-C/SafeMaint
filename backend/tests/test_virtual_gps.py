...
def test_find_nearby_equipment_matches_equipment_within_radius() -> None:
    ...
    nearby = find_nearby_equipment(
        conveyor.latitude, conveyor.longitude, SAMPLE_EQUIPMENT_LOCATIONS, radius_m=5
    )
...
def test_find_nearby_equipment_returns_nothing_far_away() -> None:
    nearby = find_nearby_equipment(0.0, 0.0, SAMPLE_EQUIPMENT_LOCATIONS, radius_m=100)
...
def test_find_nearby_equipment_sorts_by_distance() -> None:
    ...
    nearby = find_nearby_equipment(
        panel.latitude, panel.longitude, SAMPLE_EQUIPMENT_LOCATIONS, radius_m=1000
    )
...
def test_resolve_locations_without_origin_returns_original_coordinates() -> None:
    assert resolve_locations(SAMPLE_EQUIPMENT_LOCATIONS, None) == SAMPLE_EQUIPMENT_LOCATIONS


def test_resolve_locations_shifts_whole_layout_to_origin() -> None:
    origin = (0.0, 0.0)

    shifted = resolve_locations(SAMPLE_EQUIPMENT_LOCATIONS, origin)
...
def test_resolve_locations_preserves_relative_layout() -> None:
    ...
    original_distance = distance_between(SAMPLE_EQUIPMENT_LOCATIONS, "CONV-203", "PNL-01")
    shifted_distance = distance_between(
        resolve_locations(SAMPLE_EQUIPMENT_LOCATIONS, (0.0, 0.0)), "CONV-203", "PNL-01"
    )
...
def test_find_nearby_equipment_matches_calibrated_position() -> None:
    ...
    nearby = find_nearby_equipment(
        0.0, 0.0, SAMPLE_EQUIPMENT_LOCATIONS, radius_m=5, origin=origin
    )
