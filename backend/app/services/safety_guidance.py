...
def build_safety_guidance(question: str) -> SafetyGuidance:
    ...
    is_conveyor = any(
        ...
        for keyword in ("컨베이어", "콘베이어", "벨트", "풀리", "롤러")
    )
    is_bearing = any(keyword in normalized for keyword in ("베어링", "축받이", "교체"))
    is_cleaning = any(
        keyword in normalized for keyword in ("청소", "세척", "이물질", "제거")
    )
    ...
    if is_conveyor and is_bearing:
        return SafetyGuidance(
            ...
            + (
                ...
                "베어링 형식, 체결 토크, 윤활유 종류와 정렬 허용치는 해당 제조사 매뉴얼을 기준으로 확인합니다.",
            ),
            manual_note="제조사 매뉴얼이 확인되지 않으면 체결 토크·간극·윤활 조건을 임의로 결정하지 마세요.",
        )
    ...
    if is_conveyor and is_cleaning:
        return SafetyGuidance(
            ...
            + (
                ...
                "청소 도구와 세척제는 설비 재질 및 물질안전보건자료(SDS)에 맞는 것을 사용하고 바닥 오염을 즉시 제거합니다.",
            ),
            manual_note="세척제, 물 사용 가능 여부와 청소 주기는 제조사 매뉴얼을 우선 적용하세요.",
        )
    ...
    if is_conveyor:
        return SafetyGuidance(
            ...
            actions=CONVEYOR_ACTIONS,
            manual_note="부품별 분해·조립 기준은 해당 설비와 부품의 제조사 매뉴얼을 확인하세요.",
        )
...
def format_safety_answer(question: str, source_titles: list[str] | None = None) -> str:
    ...
    actions = "\n".join(
        f"{index}. {action}" for index, action in enumerate(guidance.actions, start=1)
    )
    source_note = ""
    if source_titles:
    ...
    return (
        ...
        f"작업 전 확인사항:\n{actions}\n\n"
        f"{guidance.manual_note}{source_note}\n\n"
        "이 안내는 작업 승인이 아닙니다. 현장 상태와 제조사 매뉴얼을 확인하고 "
        "안전관리자의 최종 승인 전에는 작업을 시작하지 마세요."
    )
